#include "AdvancedQuizPanel.h"
#include "../App.h"
#include <fstream>
#include "../rc/social_skills_unique_new.h"
#include "../external/json.hpp"
#include <wx/graphics.h>
void AdvancedQuizPanel::ApplyTheme() {
...
AdvancedQuizPanel::AdvancedQuizPanel(wxWindow* parent)
    ...
    explainButton->Bind(wxEVT_BUTTON, [this](wxCommandEvent&) {
        ...
        if (frame && frame->chatbotPanel && frame->chatbotPanel->inputCtrl) {
            if (frame->chatbotPanel->newBtn->IsEnabled()) {
                frame->chatbotPanel->inputCtrl->SetValue("Using the Explain More button, from the advanced quiz, explain the correct answer and why: " + questionText->GetLabel() + ". The choices are A. " + choiceButtons[0]->GetLabel() + ", B. " + choiceButtons[1]->GetLabel() + ", C. " + choiceButtons[2]->GetLabel() + ", and D. " + choiceButtons[3]->GetLabel() + " The answer selected was: " + choiceButtons[selectedIndex]->GetLabel());
    ...
    submitButton->Bind(wxEVT_BUTTON, &AdvancedQuizPanel::OnSubmit, this);
}

void AdvancedQuizPanel::LoadQuestions() {
    // Parse JSON using nlohmann::json
    nlohmann::json root = nlohmann::json::parse(std::string(AdvancedjsonData));
...
void AdvancedQuizPanel::ShowNextQuestion(wxCommandEvent& event) {
    if (currentQuestionIndex >= quizQuestions.size()) {
        ...
        wxString guidance;
        if (pct >= 90) {
            guidance = "🌟 Amazing! You're a social reasoning superstar! "
                       "Try exploring the Open-Ended Quiz to challenge yourself even further.";
        } else if (pct >= 70) {
            guidance = "👍 Great job! You have strong social skills. "
                       "You might want to revisit the Social Skills section for some advanced tips.";
        } else if (pct >= 50) {
            guidance = "🙂 Not bad! You understand some concepts but could use more practice. "
                       "Try reviewing specific lessons or asking the chatbot for explanations.";
        } else {
            guidance = "⚠️ Looks like this was challenging. Don't worry — you can learn from mistakes. "
                       "Consider revisiting the Social Skills lessons or chatting with the bot for help.";
        }
    ...
    // Existing code for showing next question
    submitButton->SetLabel("Submit");
    submitButton->Unbind(wxEVT_BUTTON, &AdvancedQuizPanel::ShowNextQuestion, this);
    ...
    explanationText->SetLabel("");
    explainButton->Hide();
    if (currentQuestionIndex >= 0 && dynamic_cast<MyFrame*>(wxGetTopLevelParent(this))->advancedQuizPanelCount >= 1) {
...
void AdvancedQuizPanel::OnSubmit(wxCommandEvent& event) {
    if (currentQuestionIndex >= quizQuestions.size())
        return;
    ...
    submitButton->Fit();
    submitButton->Bind(wxEVT_BUTTON, &AdvancedQuizPanel::ShowNextQuestion, this);
}
